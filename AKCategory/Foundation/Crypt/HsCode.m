...
//	}
    ...
    if ([key1 length] <8) {
        for (int i=0; i<(key1.length-8); i++) {
            key1=[NSString stringWithFormat:@"%@0",key1];
        }
    }
    if ([key2 length] <8) {
        for (int i=0; i<(key2.length-8); i++) {
            key2=[NSString stringWithFormat:@"%@0",key2];
        }
    }
    if ([key3 length]<8) {
        for (int i=0; i<(key3.length-8); i++) {
            key3=[NSString stringWithFormat:@"%@0",key3];
        }
    }
    ...
    return strText;//[strText autorelease];
...
//	}
    ...
    return  baseStr;//[baseStr autorelease];
...
//	NSData * data = [Base64 Base64EncodeWithChars:srcbt3 length:outLength];
    NSData * data = [NSData dataWithBytes:srcbt3 length:outLength];
	free(srcbt3);
	return data;
}
...
//	
//	NSString * baseStr = [[NSString alloc] initWithBytesNoCopy:srcbt3 length:outLength encoding:NSUTF8StringEncoding freeWhenDone:YES];
    NSData *outdata = [NSData dataWithBytes:srcbt3 length:outLength];
	free(srcbt3);
	return outdata;
}
